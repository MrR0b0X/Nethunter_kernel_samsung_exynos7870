...
/* Forward declaration: vos_cnss.h only takes pointers to struct pci_dev,
 * so we don't need to pull in <linux/pci.h> here. Without this, compilers
 * on kernels/configs that don't transitively include pci.h emit:
 *   error: 'struct pci_dev' declared inside parameter list
 */
struct pci_dev;
